    ...
    <Container>
      <VStack>
        ...
        >
          ...
          <Image src={image} h="200" objectFit="scale-down" />
        </Box>
        {/* 解答部分 */}
...
const TestNumberSlider = ({ testLen, setTestNo }: TestNumberSliderProps) => {
  return (
    <Box mx="auto" w="20%" bg="green.100" borderRadius="20" p="5">
      <Text>問題を選択</Text>
      <NumberInput
        defaultValue={0}
        min={0}
...
function makeTestPage() {
  ...
  return (
    <DefaultLayout>
      ...
      <TestNumberSlider testLen={testData.length} setTestNo={setTestNumber} />
      <AddTestMenuModal>
        <AddTestMenu
          AddTestFunc={AddTestData}
          containerProps={{
